...
class SettingsPage(QWidget):
    def __init__(self, provider: GuiDataProvider):
        ...
        root.setSpacing(14)
        root.addWidget(PageHeader("设置", "配置模型、采集、隐私、YASB 联动和日志保留策略"))
        ...
        yasb.form.addWidget(QLabel("状态 JSON"), 1, 0)
        self.status_json_path = QLineEdit()
        yasb.form.addWidget(self.status_json_path, 1, 1)
        yasb.form.addWidget(QLabel("设置文件"), 2, 0)
        self.settings_path_line = QLineEdit(str(get_settings_path()))
        self.settings_path_line.setReadOnly(True)
        yasb.form.addWidget(self.settings_path_line, 2, 1)
    ...
    def load(self) -> None:
        ...
        self.status_json_path.setText(settings.yasb.status_json_path)
    ...
    def current_settings(self) -> LocalSettings:
        ...
        settings.yasb.status_json_path = self.status_json_path.text().strip()
